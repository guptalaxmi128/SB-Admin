...
import { addInstructorCourse } from "../../../actions/instructor/course/course";
import { getInstructorCategory } from "../../../actions/instructor/category/category";
...
const Course = () => {
  ...
  const category = useSelector((state) => state.instructorCategory.category);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        await Promise.all([dispatch(getInstructorCategory())]);
      } catch (error) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      const res = await dispatch(addInstructorCourse(formData));
      if (res.success) {
