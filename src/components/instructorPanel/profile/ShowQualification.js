...
import { Layout, Menu, message, Breadcrumb, Spin, Button } from "antd";
...
import { getInstructorQualificationById } from "../../../actions/instructor/register/register";
...
const ShowQualification = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await dispatch(getInstructorQualificationById(id));
        setData(res.data);
      } finally {
        setLoading(false);
      }
                      ...
                      <div className="grid grid-cols-6 gap-4">
                        <div className="col-span-6 md:col-span-4">
                          <h1 className="text-lg font-poppins font-semibold">
                            {data.course}{" "}
                            <span className="text-xs text-slate-500 font-poppins">{`(${data.courseType})`}</span>
                          </h1>
                          <p className="text-sm text-slate-500 font-poppins">
                            By {data.university_institute_name}
                          </p>
                          <p className="text-xs text-slate-500 font-poppins">
                            {data.marksType} - {data.marks}
                          </p>
                          ...
                          <Button
                            ...
                            className={`my-8 ${
                              data.approvalStatusByAdmin === "Pending"
                                ? "border-red-500 text-red-500 font-poppins text-xs"
                                : data.approvalStatusByAdmin === "Rejected"
                                ? "border-red-500 text-red-500 font-poppins text-xs"
                                : data.approvalStatusByAdmin === "Approved"
                                ? "border-green-500 text-green-500 font-poppins text-xs"
                                : ""
                            }`}
                          >
                        ...
                        <div className="col-span-6 md:col-span-2">
                          ...
                          <p className="text-xs text-slate-900 font-poppins">
                            Duration - {data.year}
                          </p>
                        </div>
                      </div>
