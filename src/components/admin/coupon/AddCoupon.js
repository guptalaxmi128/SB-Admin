...
import { couponToCourse, getCoupon } from "../../../actions/admin/coupon/coupon";
...
const AddCoupon = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await dispatch(getCoupon());
        const approvedCoupons = res?.data.filter(coupon => coupon.approvalStatusByAdmin === 'Approved');
  ...
  const onFinish = async (values) => {
    setLoading(true);
    try {
      const res = await dispatch(couponToCourse(values));
      if (res.success) {
